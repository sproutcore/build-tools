...
BT.Installer = SC.Object.extend({
  ...
  gitCheckOut: function (url) {
    ...
    BT.Logger.info("Checking out %@ into %@".fmt(url.href, tmpPath));
    // we don't do BT.AsyncWrapper because for some reason it causes errors with child_process.exec
    require('child_process').exec(cmd, function (error, stdout, stderr) {
      if (error) {
        BT.Logger.error("error when checking out: " + require('util').inspect(error));
      }
      else {
  ...
  gitCheckOutDidFinish: function (result, stdout, stderr, url, tmpPath, folderName) {
    BT.Logger.debug("BT.Installer.gitCheckOutDidFinish: " + require('util').inspect(arguments));
    var pathlib = require('path');
    if (SC.ok(result)) {
      // we're checked out, so now copy the files into the target
      BT.Logger.info("Checkout complete...");
      var dest = pathlib.join(this.destination, folderName);
      BT.Logger.info("Copying into %@".fmt(dest));
      this.installFramework(tmpPath, dest);
      // if this finishes and doesn't crash, we are done copying :)
      // remove the tmp dir
      require('rimraf').rimrafSync(tmpPath);
      BT.Logger.info("Done copying. Don't forget to configure the new framework.");
    }
    else {
      BT.Logger.error("There was an error checking out the repository: \n" + stderr);
    }
  },
  ...
  installFramework: function (from, to) {
    ...
    var copyDir = function (dir, target) {
      ...
      fileList.forEach(function (fn) {
        ...
        if (stat.isFile()) {
          //copy file
          BT.Logger.debug("copying %@ to %@".fmt(p, t));
...
BT.startInstall = function (giturl, opts) {
  ...
  if (opts.isGlobal) { // make sure the global frameworks dir exists
    BT.mkdir("frameworks", BT.btPath);
  }
  else {
    BT.mkdir("frameworks", BT.projectPath);
  }
  if (opts.destination) { // if given an override, override the destination
    dest = opts.destination;
  }
  BT.Logger.logOutputLevel = opts.logLevel;
...
BT._installDidFinish = function () {
  BT.Logger.debug("BT._installDidFinish: " + require('util').inspect(arguments));
};
