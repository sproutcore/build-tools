...
BT.Installer = SC.Object.extend({
  ...
  gitCheckOut: function (url) {
    ...
    if (!this.silent) SC.Logger.info("Checking out %@ into %@".fmt(url.href, tmpPath));
    // we don't do BT.AsyncWrapper because for some reason it causes errors with child_process.exec
    require('child_process').exec(cmd, function (error, stdout, stderr) {
      if (error) {
        SC.Logger.error("error when checking out: " + require('util').inspect(error));
      }
      else {
  ...
  gitCheckOutDidFinish: function (result, stdout, stderr, url, tmpPath, folderName) {
    //SC.Logger.info("BT.Installer.gitCheckOutDidFinish: " + require('util').inspect(arguments));
    var pathlib = require('path');
    if (SC.ok(result)) {
      // we're checked out, so now copy the files into the target
      if (!this.silent) SC.Logger.info("Checkout complete...");
      var dest = pathlib.join(this.destination, folderName);
      if (!this.silent) SC.Logger.info("Copying into %@".fmt(dest));
      this.installFramework(tmpPath, dest);
      // if this finishes and doesn't crash, we are done copying :)
      // remove the tmp dir
      require('rimraf').sync(tmpPath);
      if (!this.silent) SC.Logger.info("Done copying. Don't forget to configure the new framework.");
    }
    else {
      SC.Logger.error("There was an error checking out the repository: \n" + stderr);
    }
  },
  ...
  installFramework: function (from, to) {
    ...
    var copyDir = function (dir, target) {
      ...
      fileList.forEach(function (fn) {
        ...
        if (stat.isFile()) {
          //copy file
          //SC.Logger.info("copying %@ to %@".fmt(p, t));
...
BT._installDidFinish = function () {
  //SC.Logger.info("BT._installDidFinish: " + require('util').inspect(arguments));
};