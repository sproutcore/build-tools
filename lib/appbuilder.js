...
//
  ...
  */
  ...
  combineFrameworks: true, // Combine the frameworks with app in build mode
  ...
  */
  html5ManifestUrl: function () {
    return '/'+this.get('name')+'/manifest.appcache';
  }.property().cacheable(),
  ...
  scripts: function () {
    var scripts = this._fws.getEach('scripts').flatten().filterProperty('content');

    if (this.get('combineFrameworks')) {
      scripts = BT.CombinedFrameworksController.create({
        framework: this._appfw,
        minify: this.get('minifyScripts'),
        outputFileClass: BT.ScriptFile,
        relpath: BT.path2Url(this.get('name') + ".js"),
        contentType: 'application/javascript',
        filesToCombine: scripts,
      });
  ...
  buildFiles: function () {
    return this.get('scripts').concat(this.get('stylesheets')).concat(this.get('moduleScripts')).concat(this.get('moduleStylesheets')).flatten();
  }.property(),
      ...
      ret = ejs.render(template.toString(), {
        ...
        sc_static: function (fn) {
          ...
          if (file.get('length') !== 1) {
            SC.Logger.log("no resource found for: " + fn);
            return '';
          }
          file = file[0];
          ...
          if (!deps) {
            appfw.setPath('scripts.firstObject.resourceDependencies', []);
            deps = appfw.getPath('scripts.firstObject.resourceDependencies');
          }
          deps.push(url);
          return url;
