...
BT.AppBuilder = SC.Object.extend({
  ...
  _instantiateFrameworks: function(){
    ...
    SC.Logger.log("deps for %@ are: %@".fmt(this.get('name'), SC.inspect(deps)));
  ...
  indexHtml: function(){
    ...
    stylesheets.forEach(function(ssh){
      ssh.forEach(function(s){
        if(s) ret.push('<link href="%@" rel="stylesheet" type="text/css">'.fmt(BT.path.join(appname,s)));
      });
    });
    ret.push("</head>");
    ret.push("<body>");
    var scripts = this._fws.getEach('scripts').flatten();
    //BT.util.log("scripts for " + this.get('name') + ": " + BT.util.inspect( scripts ));
    scripts.forEach(function(scs){
      scs.forEach(function(s){
        if(s) ret.push('<script type="text/javascript" src="%@"></script>'.fmt(BT.path.join(appname,s)));
      });
    });
  ...
  fileFor: function(url){
    ...
    for(var i=0,len=this._fws.length;i<len;i+=1){
      f = (this._fws[i] && this._fws[i].files)? this._fws[i].files[fwurl]: null;
      if(f){
        return f;
      }
    }
