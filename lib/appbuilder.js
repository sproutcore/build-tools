...
BT.AppBuilder = SC.Object.extend({
  ...
  */
  ...
  /**
    Set it to true is you want to includes the test files.
  ...
  */
  ...
  /**
    The HTML5 manifest file name to use.

    @property
    @type String
    @default 'manifest.appcache'
  */
  html5ManifestFileName: 'manifest.appcache',
  ...
  */
  ...
  /**
    if you need to use window.onload, set to true

    @property
    @type Boolean
    @default false
  */
  useWindowOnLoad: false,
  ...
  */
  ...
  /**
    no clue what this should be used for
  ...
  */
  themeName: function () {
    ...
    if (!themefw) {
      BT.Logger.warn("The template tried to insert the css theme name you defined for the theme, but the theme framework cannot be found");
  ...
  */
  indexHtmlFile: function () {
    BT.Logger.debug("indexHtmlFile in %@ called for the %@ time".fmt(this.get('name'), (!!this._indexFile ? "second": "first")));
    if (!this._indexFile) {
  ...
  */
  wrapTest: function (testFile) {
    ...
    BT.Logger.trace("debugRef: " + util.inspect(debugRef));
    ...
    var findDeps = function (fwd) {
      ...
      if (!ret) BT.Logger.warn("Something is fishy, because cannot find a instance for ref");
      if (ret.dependencies) {
    ...
    if (!fws.findProperty('ref', debugRef)) {
      BT.Logger.warn("cannot find debug ref in fws, pushing");
      tmp = this._fws.findProperty('ref', debugRef);
      if (!tmp) {
        BT.Logger.warn("all fws: " + this._fws.getEach('ref'));
        BT.Logger.warn("this app: " + this.get('name'));

      }
      BT.Logger.debug("tmp is " + tmp.get('ref'));
      fws.push(tmp);
    }
    if (!fws.findProperty('ref', testingRef)) {
      BT.Logger.warn("cannot find testingRef in fws, pushing");
