...
BT.AppBuilder = SC.Object.extend({
  ...
  indexHtml: function () {
    ...
    ret.push("<body>");
    if (this.includeSC) {
      ret.push('<script type="text/javascript"> var SC = window.SC || {} </script>');
    }
    var scripts = this._fws.getEach('scripts');
    // adding bootstrap system
    this._bootstrap.scripts.forEach(function (f) {
