...
BT.AppBuilder = SC.Object.extend({
  ...
  */
  ...
  /**
    The available environment names.

    In your app, the code between `// @if (env.X)` and `// @endif`
    will be preserved if X match the environment name.
  ...
  */
  init: function () {
    ...
    if (!this.envNames.contains(BT.envName)) BT.Logger.warn("Environment '%@' is unknow. Allowed environments are %@".fmt(BT.envName, JSON.stringify(this.envNames)));
    else BT.Logger.info("Environment is '%@'".fmt(BT.envName));
