...
BT.AppBuilder = SC.Object.extend({
  ...
  */
  languagesToBuild: function () {
    ...
    if (this.get('relativeBuild')) {
      if (languages.get('length') > 1) {
        var language = languages[0];
        languages = [language];

        SC.Logger.warn("Only one language can be build in relative mode. The language '%@' has been selected.".fmt(language));

        // To prevent other warnings
