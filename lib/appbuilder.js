  ...
  _registerFrameworks: function () {
    ...
    themedeps.forEach(function (tref, td_i) {
      ...
      if (SC.typeOf(tref) === SC.T_STRING) {
        ...
        if (k) i = k.create({ belongsTo: this });
      }
      else {
        k = BT.projectManager.getThemeClass(tref.ref);
        if (k) i = k.create(tref[BT.runMode], { belongsTo: this });
      }
      if (i) {
        this._fws.push(i);
    ...
    BT.Logger.trace('Setting app._allFws with ', this._fws);
    ...
    BT.Logger.trace('Done instantiating frameworks');
  },
  ...
  */
  fileFor: function (url) {
    ...
    if (url === this.get('html5ManifestUrl')) {
      //TODO: this needs a check whether it is actually dealing with a manifest url
      //because this could also be called when both are undefined for some reason.
