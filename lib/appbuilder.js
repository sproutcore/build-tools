...
//
  ...
  */
  ...
  init: function () {
    ...
    if (this.doNotBuild && BT.runMode === BT.RM_BUILD) return;
  ...
  indexHtmlFile: function () {
    ...
    if (!this._indexFile) {
      ...
      this._indexFile.content = this.get('indexHtml');
    }
    return this._indexFile;
  }.property(),

  indexHtmlUrl: function () {
    // a very thin line walking here: the first time indexHtmlFile is generated it doesn't have a content
    // this is normally enough to generate the url.
    // however, if somehow people want to have the content hash in the url, this might cause an endless loop...
    return this.get('indexHtmlFile').get('url');
