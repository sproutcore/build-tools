...
BT.File = SC.Object.extend({
      ...
      while (match = scstaticRegex.exec(line)) {
        ...
        if (isLastStageInBuild) {
          ...
          if (fn.indexOf(":") > -1) {
            ...
            if (!crossFw) {
              SC.Logger.warn(klassName + "#handleStatic: %@ is referred in %@ but this framework is still not loaded.".fmt(matchText, this.get('path')));
        ...
        if (res.length === 0) {
          SC.Logger.warn(klassName + "#handleStatic: found no files for %@ in file %@".fmt(matchText, this.get('path')));
          line = line.replace(matchText, '""'); // replace with empty string...
          continue;
        }
        else if (res.length > 1) {
          SC.Logger.warn(klassName + "#handleStatic: found multiple files for %@ in file %@, taking the first (%@)".fmt(matchText, this.get('path'), f.get('path')));
        }
        f = res[0];
        ...
        SC.Logger.debug("#handleStatic: %@ has been replaced in %@.".fmt(matchText, line));
      }
  ...
  _addBtScstaticMarker: function (line, matchText, matchIndex, separator) {
    if (line.indexOf("BT_SCSTATIC") === -1) {
      ...
      console.log('_addBtScstaticMarker', line+"\n");
    }
  ...
  _removeBtScstaticMarker: function (line, matchText, matchIndex, separator) {
    if (line.indexOf("BT_SCSTATIC") > -1) {
      ...
      console.log('_removeBtScstaticMarker', line+"\n");
    }
