...
BT.File = SC.Object.extend({
  ...
  appUrl: function () {
    return this.getPath('framework.belongsTo.url');
  }.property(),
  ...
  frameworkBuildNumber: function () {
    return this.getPath('framework.buildNumber');
  }.property(),
  ...
  _filterResourceDependencies: function (fn) {
    ...
    return res.filter(function (file) {
      ...
      var p = file.get('path');
      return p.indexOf(newfn, p.length - newfn.length) > -1;
    });
  },
