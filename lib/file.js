...
BT.File = SC.Object.extend({
  ...
  url: function () {
    ...
    var opts = {
      ...
      language: framework.getPath('belongsTo.language'),
  ...
  handleStatic: function (file, createRes) {
    ...
    var ret = [];
    var useStaticResourceUrl = this.useStaticResourceUrl;
    lines.forEach(function (line) {
      ...
      if (match) {
        ...
        if (fn.indexOf(":") > -1) {
          ...
          if (!crossFw) {
            SC.Logger.log(klassName+"#handleStatic: %@ is referred in %@ but this framework is still not loaded.".fmt(match[0], this.get('path')));
        ...
        if (opts.length === 0) {
          SC.Logger.log(klassName+"#handleStatic: found no files for %@ in file %@".fmt(match[0], this.get('path')));
          ret.push(line.replace(scstaticRegex, '""')); // replace with empty string...
          return;
        }
        // still running?
        f = opts[0];
        if (opts.length > 1) {
          SC.Logger.log(klassName+"#handleStatic: found multiple files for %@ in file %@, taking the first (%@)".fmt(match[0], this.get('path'), f.get('path')));
        }
        // now we have a match, we need an url
        var replaceURL = f.get('url');

        if (useStaticResourceUrl) ret.push(line.replace(scstaticRegex, 'url("%@")'.fmt(replaceURL)));
        else ret.push(line.replace(scstaticRegex, '"%@"'.fmt(replaceURL)));
        ...
        resourceDeps.push({ path: f.get('path'), url: replaceURL });
      }
      else {
        ret.push(line);
      }
