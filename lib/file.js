...
BT.File = SC.Object.extend({
  ...
  handleStatic: function (file, opts) {
    ...
    lines.forEach(function (line) {
      var match = true, scstatic, fn, res, f, sep, fwForFile;
      ...
      while (match) {
        ...
        if (match) {
          ...
          if (fn.indexOf(":") > -1) {
            ...
            if (!crossFw) {
              SC.Logger.warn(klassName+"#handleStatic: %@ is referred in %@ but this framework is still not loaded.".fmt(scstatic, this.get('path')));
              line = line.replace(scstaticRegex, '""');
              return;
            }
            fwForFile = crossFw;
            fn = newfn;
          ...
          sep = scstatic.indexOf('"') === -1 ? "'" : '"';
          res = this.findFwResourceFor(fwForFile, fn, opts);
          
          if (res.length === 0) {
            SC.Logger.warn(klassName+"#handleStatic: found no files for %@ in file %@".fmt(scstatic, this.get('path')));
            line = line.replace(scstaticRegex, '""');
            return;
          }
          if (res.length > 1) {
            SC.Logger.warn(klassName+"#handleStatic: found multiple files for %@ in file %@, taking the first (%@)".fmt(scstatic, this.get('path'), f.get('path')));
          }
          f = res[0];
          line = this.replaceResourceFromLine(line, f, sep, scstaticRegex, opts);
  ...
  replaceResourceFromLine: function (line, file, sep, scstaticRegex, opts) {
    var url = file.get('url');
    return line.replace(scstaticRegex, (sep + "%@" + sep).fmt(url));
  }
