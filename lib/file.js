...
BT.File = SC.Object.extend({
  ...
  contentHash: function () {
    if (BT.runBenchmarks) SC.Benchmark.start('File:contentHash');
    var crypto = require('crypto');
    var c = this.get('rawContent');
    if (c) {
      return crypto.createHash('sha1').update(c, 'utf8').digest('hex');
    }
    if (BT.runBenchmarks) SC.Benchmark.end('File:contentHash');
  }.property('rawContent').cacheable(),
    ...
    lines.forEach(function (line, line_index) {
      ...
      while (match = scstaticRegex.exec(line)) {
        ...
        if (res.length === 0) {
          ...
          BT.Logger.warn(klassName + "#handleStatic: found multiple files for %@ in file %@, taking the first (%@)".fmt(matchText, this.get('path'), res[0].get('path')));
        }
        f = res[0];
