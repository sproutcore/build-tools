...
BT.projectManager = SC.Object.create({
  ...
  startBuild: function () {
    ...
    var apps = Object.keys(this.apps).map(function (appname) { return this.apps[appname]; }, this);
    var files = apps.getEach('files').flatten().uniqueProperty('url');
    // save files :)
    var pathlib = require('path');
    var fslib = require('fs');
    ...
    files.forEach(function (file) {
      var p = pathlib.join(basepath, file.get('url'));
      this._mkdir(pathlib.dirname(p), basepath);
