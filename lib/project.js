...
BT.projectManager = SC.Object.create({
  ...
  _startBuild: function () {
    ...
    apps.getEach('scripts').concat(apps.getEach('stylesheets')).flatten().forEach(function (f) {
      files.push(f);
      var resourceDeps = f.get('resourceDependencies');
      if (resourceDeps && resourceDeps.get('length') > 0) {
        resourceDeps.forEach(files.push);
      }
    });
    ...
    var pathlib = require('path');
    var fslib = require('fs');
    this._mkdir("build", BT.projectPath);
    ...
    files.forEach(function (file, i) {
      ...
      this._mkdir(pathlib.dirname(urlPath), basepath);
    ...
    // combine frameworks with the app if needed
    apps.forEach(function (app) {
      if (app.get('combineFrameworks')) {
        var urlPath = BT.url2Path(app._appfw.getPath('scripts.firstObject.url'));
        var p = pathlib.join(basepath, urlPath);
        var appScripts = [];
        app.get('scripts').forEach(function(script) {
          appScripts.push(script.get('content'));
        }, this);
        fslib.writeFileSync(p, appScripts.join("\n"));

        urlPath = BT.url2Path(app._appfw.getPath('stylesheets.firstObject.url'));
        p = pathlib.join(basepath, urlPath);
        var appStylesheets = [];
        app.get('stylesheets').forEach(function(stylesheets) {
          appStylesheets.push(stylesheets.get('content'));
        }, this);
        fslib.writeFileSync(p, appStylesheets.join("\n"));
      }
    });
