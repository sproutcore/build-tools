...
BT.projectManager = SC.Object.create({
  ...
  startServer: function () {
    SC.Logger.info("Starting development server...");
    ...
    if (proxyConf) {
      //SC.Logger.info("creating proxies");
      this._proxies = [];
      proxyConf.forEach(function (p) {
        //SC.Logger.info("feeding proxy: " + require('util').inspect(p));
        this._proxies.push(BT.Proxy.create(p));
      }, this);
      //this._proxies = proxyConf.map(BT.Proxy.create);
    }

    if (BT.serverConfig && BT.serverConfig.localOnly === false) {
      hostname = [];
      var os = require('os');
    ...
    http.createServer(f).on('error', function (err) {
      if (err) {
        if (err.code === "EOF") {
          SC.Logger.error("Error while trying to attach the server. Is the port perhaps taken?");
          process.exit(1);
        }
        else {
          SC.Logger.error('Unknown error while trying to attach the server.');
          process.exit(1);
        }
      }
    }).listen(port, hostname, function () {
      var serverUrl = url.format({ protocol: 'http', hostname: hostname, port: port});
      SC.Logger.info("Server started on " + serverUrl);
      if (BT.runBenchmarks) {
      ...
      if (BT.debugServer) {
        ...
        SC.Logger.debug("BT.debugServer, writing out files");
        ...
        Object.keys(me.apps).forEach(function (appname) {
          ...
          appfiles.forEach(function (f) {
            ...
            SC.Logger.debug("relpath: " + relpath);
        ...
        SC.Logger.debug("done writing files...");
      }
    });
  },
  ...
  _startBuild: function () {
    ...
    if (opts && opts.apps && opts.apps.length > 0) {
      opts.apps.forEach(function (appname) {
        ...
        if (!app) SC.Logger.warn("Could not find app '%@'".fmt(appname));
        else {
          apps.push(app);
        }
      }, this);
    ...
    if (this.appsToBuild.length === 0) {
      SC.Logger.warn("No apps found with this name");
      process.exit(1);
    }
    ...
    if (unloadedFile) {
      SC.Logger.warn("postponing save, the apps are not ready yet: " + unloadedFile.get('path'));
      this.invokeNext('_startBuild');
      return;
    }

    SC.Logger.info("Building %@ app(s): %@".fmt(apps.get('length'), apps.getEach('name')));

    this.buildLanguages = SC.Set.create();
    apps.forEach(function (app) {
      this.buildLanguages.addEach(app.get('languagesToBuild'));
    }, this);
  ...
  _startBuildLanguage: function () {
    SC.Logger.debug("_startBuildLanguage");
    var language = this.buildLanguage = this.buildLanguages.pop();
    if (!language) return this._endBuild();

    this.appsToBuild.forEach(function (app) {
      if (app.get('languagesToBuild').contains(language)) {
  ...
  _doBuildLanguage: function () {
    ...
    SC.Logger.debug("_doBuildLanguage for " + this.buildLanguage);

    apps.getEach('files').flatten().forEach(function (f) {
      allResources.push(f);
    });

    SC.Logger.debug("allResources pushed");

    apps.forEach(function (app) {
      files.push(app.get('indexHtmlFile'));
      SC.Logger.debug("adding index.html for " + app.get('name'));
      if (app.get('addHtml5Manifest')) {
        SC.Logger.debug("added html5 manifest file for " + app.get('name'));
        files.push(app.get('html5ManifestFile'));
      }
    });

    SC.Logger.debug("added index.html for all apps");
    ...
    SC.Logger.debug("added buildFiles");

    SC.Logger.debug("Calculating file contents");
    //SC.Logger.info("num files before filtering: " + files.get('length'));
    var urls = [];
    ...
    //SC.Logger.debug("num files after filtering: " + files.get('length'));
    //files = files.uniqueProperty('url'); // filter based on uniqueness... uniqueProperty doesn't exist...
    // save files :)
    SC.Logger.info("Saving '%@' files...".fmt(language));
    var pathlib = require('path');
    var fslib = require('fs');
    ...
    files.forEach(function (file, i) {
      ...
      SC.Logger.debug("About to save %@".fmt(file.get('url')));
      ...
      if (resourceDeps && resourceDeps.get('length') > 0) {
        resourceDeps.forEach(function (dep) {
          ...
          if (fslib.statSync(depPath).isFile()) {
            ...
            SC.Logger.error("the file %@ doesn't exist".fmt(depPath));
          }
        }, this);
      }
      SC.Logger.debug("saved file %@ of %@".fmt(i, files.length));
      //SC.Logger.debug("File %@ written to %@".fmt(file.get('url'), p));
    }, this);
  ...
  _endBuild: function () {
    ...
    SC.Logger.log("Build time %@ minutes %@ secs".fmt(minutes, seconds));
    // throw new Error("QUIT"); // seems
    if (BT.runBenchmarks) {
      SC.Benchmark.end("build");
      SC.Logger.log(SC.Benchmark.report());
    }
    if (!this._buildOpts.REPL) process.exit(0);
  },
  ...
  onRequest: function (req, res) {
    ...
    var appnames = Object.keys(this.apps);
    var i;
    //SC.Logger.info("http request for: " + path);
    if (path === "/") { // root index
      ...
      appnames.forEach(function (a) {
        ...
        if (path.slice(1).indexOf(a) === 0) {
          ...
          if (c) {
            ...
            if (!content) {
              //SC.Logger.info("file %@ doesn't have content?".fmt(c.get('path')));
              res.write("");
            }
  ...
  addApp: function (app) {
    ...
    SC.Logger.info("Adding app '%@'".fmt(appname));
  ...
  getFrameworkClass: function (fwref) {
    var pathlib = require('path');
    //SC.Logger.info('trying to find class for ' + fwthis.gsub(filecontent, INCLUDE, 'replacer'););
    ...
    if (!ret) { // try to actively locate the fw
      ...
      //SC.Logger.info("trying to runConfig: " + d);
      var loadresult = sc_require(d);
      //SC.Logger.info("loadresult: " + require('util').inspect(loadresult));
      if (!SC.ok(loadresult) && loadresult.code === "ENOENT") {
        //SC.Logger.info("not present in project, trying to load from global folder");
