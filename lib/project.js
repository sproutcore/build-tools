...
BT.projectManager = SC.Object.create({
  ...
  _startBuild: function () {
    ...
    apps.getEach('scripts').concat(apps.getEach('stylesheets')).concat(apps.getEach('moduleScripts')).concat(apps.getEach('moduleStylesheets')).flatten().forEach(function (f) {
      files.push(f);
    });
    ...
    files.forEach(function (file, i) {
      ...
      if (resourceDeps && resourceDeps.get('length') > 0) {
        resourceDeps.forEach(function(dep) {
          this._mkdir(pathlib.dirname(dep.url), basepath);
          
          if (fslib.statSync(dep.path).isFile()) {
            fslib.writeFileSync(pathlib.join(basepath, dep.url), fslib.readFileSync(dep.path));
          }
          else {
            SC.Logger.log("the file %@ doesn't exist".fmt(dep.path));
          }
        }, this);
      }
