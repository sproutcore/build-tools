...
BT.projectManager = SC.Object.create({
  ...
  startServer: function () {
    ...
    http.createServer(f).on('error', function (err) {
      ...
      if (BT.debugServer) {
        ...
        BT.Logger.debug("BT.debugServer, writing out files");
        ...
        Object.keys(me.apps).forEach(function (appname) {
          ...
          appfiles.forEach(function (f) {
            ...
            BT.Logger.debug("relpath: " + relpath);
  ...
  _startBuildLanguage: function () {
    BT.Logger.debug("_startBuildLanguage");
  ...
  _doBuildLanguage: function () {
    ...
    BT.Logger.debug("_doBuildLanguage for " + this.buildLanguage);

    apps.getEach('files').flatten().forEach(function (f) {
      allResources.push(f);
    });

    BT.Logger.debug("did push allResources pushed");

    apps.forEach(function (app) {
      files.push(app.get('indexHtmlFile'));
      BT.Logger.debug("adding index.html for " + app.get('name'));
      if (app.get('addHtml5Manifest')) {
        BT.Logger.debug("added html5 manifest file for " + app.get('name'));
        files.push(app.get('html5ManifestFile'));
      }
    });

    BT.Logger.debug("added index.html for all apps");
    ...
    BT.Logger.debug("num files before filtering: " + files.get('length'));
    var urls = [];
    ...
    BT.Logger.debug("num files after filtering: " + files.get('length'));
    ...
    files.forEach(function (file, i) {
      ...
      if (fslib.existsSync(p) && !BT.OVERWRITE_BUILD) {
        BT.Logger.info("File (%@ of %@) %@ already written to %@".fmt(i+1, files.length, file.get('url'), p));
        return;
      }
  ...
  onRequest: function (req, res) {
    ...
    if (!hasServed) {
      ...
      if (i > -1) {
        var index = this.apps[appnames[i]].getPath('indexHtmlFile.content');
        if (index) {
          res.writeHead(200);
          res.write(index);
          res.end();
  ...
  addFramework: function (fwclass) {
    ...
    BT.Logger.trace('registering framework: ' + fwref);
