  ...
  _doBuildLanguage: function () {
    ...
    files.forEach(function (file, i) {
      ...
      if (resourceDeps && resourceDeps.get('length') > 0) {
        resourceDeps.forEach(function(depStaticUrl) {
          var res = allResources.findProperty('staticUrl', depStaticUrl),
            resPath = res.get('path'),
            resUrl = res.get('url');
          BT.mkdir(pathlib.dirname(resUrl), basepath);
          
          if (fslib.statSync(resPath).isFile()) {
            fslib.writeFileSync(pathlib.join(basepath, resUrl), fslib.readFileSync(resPath));
          }
          else {
            SC.Logger.log("the file %@ doesn't exist".fmt(resPath));
          }
        }, this);
      }
