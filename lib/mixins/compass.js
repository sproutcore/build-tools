...
BT.CompassMixin = {
  ...
  handleCompass: function (css) {
    ...
    for (i = 0; i < numlines; i += 1) {
      ...
      if (curline.indexOf('@include') === -1) {
        ...
        if (!replaced) {
          //SC.Logger.debug("not replaced line: " + curline);
  ...
  //     SC.Logger.debug("match for transform: " + m[2]);
  //     var ret = [];
...
//  -moz-transition-duration: 0.5s;
  ...
  //     //SC.Logger.debug("match: " + SC.inspect(m[1]));

  //     //SC.Logger.debug("match: " + m[2]);
  ...
  //     if (!sidesToInclude) {
  //       SC.Logger.info("css.js: unknown border radius type in " + this.get('path'));
  ...
  handleBorderRadius: function (m) {
    ...
    var sidesToInclude = sides[borderSide];
    if (!sidesToInclude) {
      SC.Logger.warn("css.js: unknown border radius type in " + this.get('path'));
      return m[0]; // return the original to leave it in
    }
...
//background-image: -webkit-linear-gradient(top, #eeeeee, #c0c0c0);
  ...
  handleBackground: function (m) {
    ...
    //SC.Logger.debug("basis in handleBackground: " + basis);
    SC.Logger.debug("handleBackground: %@, %@, %@".fmt(m[1], m[2], m[3]));
