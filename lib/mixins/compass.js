...
BT.CompassMixin = {
  ...
  handleCompass: function (css) {
    ...
    for (i = 0; i < numlines; i += 1) {
      ...
      if (curline.indexOf('@include') === -1) {
        ...
        if (!replaced) {
          //BT.Logger.debug("not replaced line: " + curline);
  ...
  //     BT.Logger.debug("match for transform: " + m[2]);
  //     var ret = [];
...
//  -moz-transition-duration: 0.5s;
  ...
  //     //BT.Logger.debug("match: " + SC.inspect(m[1]));

  //     //BT.Logger.debug("match: " + m[2]);
  ...
  //     if (!sidesToInclude) {
  //       BT.Logger.info("css.js: unknown border radius type in " + this.get('path'));
  ...
  handleBorderRadius: function (m) {
    ...
    var sidesToInclude = sides[borderSide];
    if (!sidesToInclude) {
      BT.Logger.warn("css.js: unknown border radius type in " + this.get('path'));
      return m[0]; // return the original to leave it in
    }
...
//background-image: -webkit-linear-gradient(top, #eeeeee, #c0c0c0);
  ...
  handleBackground: function (m) {
    ...
    //BT.Logger.debug("basis in handleBackground: " + basis);
    BT.Logger.debug("handleBackground: %@, %@, %@".fmt(m[1], m[2], m[3]));
...
// This might be a way to support @mixin parsing. It is very simple atm, doesn't support
// loops, but it can do plain variable replacement
//
// BT.registerSassMixin = function (css) {
//   var ret = {};
//   var lines = css.split("\n");
//   var insideMixinDef = false;
//   var mixinDefReg = /@mixin\s+(.+)\((.+)\)\s?\{/;
//   var curMixin = null;
//   var code = "";

//   var parseArgList = function (list) {
//     ret = [];
//     var items = list.split(",");
//     items.forEach(function (item) {
//       var arg = item.trim().split(":");
//       var argname = arg[0], defaultValue = arg[1];
//       ret.push({ argname: argname, defaultValue: defaultValue });
//     });
//   };

//   var createSassFunction = function (block) {
//     // first line is the function def
//     var blocklines = block.split("\n");
//     var m = mixinDefReg.exec(blocklines[0]);
//     var blockCode = block.slice(1, block.length - 1);
//     var args = parseArgList(m[2]);
//     return function (arglist) { // arglist is the argument list of the mixin call
//       var ret = blockCode.map(function(line){ return line; });
//       var callArgs = arglist.split(",");
//       args.forEach(function (arg, i) {
//         ret.forEach(function (l) {
//           l.replace(arg.name, (callArgs[i] || arg.defaultValue))
//         });
//       });
//       return ret.join("\n");
//     }
...
//     var match = mixinDefReg.exec(line);
//     if (match) {
//       insideMixinDef = true;
//       curMixin = match[1];
//       code = line;
...
//       code += "\n" + line;
//       if (line.indexOf("}") > -1) {
//         insideMixinDef = false;
//         ret[curMixin] = createSassFunction(code);
//         code = "";
//       }
//     }
//   });
