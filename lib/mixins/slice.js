...
BT.SliceMixin = {
  ...
  create_slice: function (opts) {
    ...
    //SC.Logger.debug('creating slice in fw ' + framework.get('name') + ' with opts: ' + SC.inspect(opts));
    var slice;
    ...
    if (!file) { // meaning both x2 and normal have failed
      SC.Logger.warn('filename not found for slice! ' + filename);
      SC.Logger.warn("filename referenced in " + this.get('path'));
      SC.Logger.warn("filename part of framework: " + framework.get('name'));
      return;
    }
    if (file.get('extension') === "gif") {
      SC.Logger.warn('You are trying to apply slicing on a gif. This is not supported');
      return;
    }
    //SC.Logger.debug("for slice in css_file: " + this.get('path') + " using file: " + file.get('path'));
    opts.file = file;
  ...
  handle_slices: function (opts) {
    //SC.Logger.debug('handle_slices: opts: ' + SC.inspect(opts));
  ...
  replacer: function (match, opts) {
    ...
    if (m.indexOf("slices") >= 0) { // slice or slices,
      ...
      //SC.Logger.debug('opts for handle_slice: ' + SC.inspect(opts));
  ...
  slice_rect: function (slice, imageWidth, imageHeight) {
    ...
    //SC.Logger.debug("slice_rect: left: %@, top: %@, bottom: %@, right: %@, width: %@, height: %@".fmt(left,top,bottom,right,width,height));
    //SC.Logger.debug('imageWidth: %@, imageHeight: %@'.fmt(imageWidth,imageHeight));
  ...
  slice_image: function (slice, img, cssname) {
    ...
    //SC.Logger.debug(slice.filename + ': image size after loading: w: %@, h: %@'.fmt(img.width,img.height));
    var mustSlice = (slice.left !== 0 || slice.right !== 0 || slice.top !== 0 || slice.bottom !== 0);

    //var t = new Canvas(10,10);
    //SC.Logger.debug("canvas is " + t.getContext.toString());
    //var l = t.getContext.call(t,'2d');

    var f = slice.proportion;
    if (mustSlice || slice.x2) {
      if (mustSlice) {
        //SC.Logger.debug('mustSlice for slice ' + slice.file.get('path'));
        rect = this.slice_rect(slice, img.width / f, img.height / f);
        if (rect) {
          //if(cssname) SC.Logger.debug(cssname + ' rect for file ' +  slice.file.get('path') + ' is: ' + SC.inspect(rect));
          ...
          if (rect.width && rect.height) { // only draw on canvas with width and height being not zero
            //SC.Logger.debug('creating new canvas with size: ' + rect.width*f + " and " + rect.height*f);
