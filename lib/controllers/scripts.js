...
BT.FrameworkScriptsController = SC.ArrayController.extend({
  ...
  filesHaveChanged: function () {
    ...
    //SC.Logger.info("script file has changed..." + ret);
  ...
  dependenciesDidChange: function () {
    ...
    if (!fw) return;
    //SC.Logger.info("dependenciesDidChange for framework: " + this.getPath('framework.name'));
    // get all deps
    var deps = {};
    ...
    if (this.getPath('framework.ref') === "sproutcore:jquery") {
      //SC.Logger.info("content of deps: %@".fmt(require('util').inspect(deps)));
    }
      ...
      // For files, check if it's been handled; if not, recurse its dependencies.
        ...
        if (!ret.contains(fileOrFolder)) {
          ...
          if (currentlyProcessingFiles.contains(fileOrFolder)) {
            ...
            SC.Logger.error("BuildTools encountered a circular dependency.");
            SC.Logger.error('The file %@ was required via sc_require(), while already being processed:'.fmt(fileOrFolder));
            currentlyProcessingFiles.forEach(function (file) { SC.Logger.error('  %@ =>'.fmt(file)); });
            SC.Logger.error('  %@'.fmt(fileOrFolder));
            SC.Logger.error('You must fix this before proceeding.'.fmt(fileOrFolder));
