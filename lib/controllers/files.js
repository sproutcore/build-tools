...
BT.FrameworkFilesController = SC.ArrayController.extend({
  ...
  sassPartials: function () {
    return this.filter(function (f) {
      return /_.+\.css/.exec(f.get('path'));
    });
  }.property('[]').cacheable(),
  ...
  contentHash: function () {
    if (BT.runBenchmarks) SC.Benchmark.start('FrameworkFilesController:contentHash');
    ...
    if (BT.runBenchmarks) SC.Benchmark.end('FrameworkFilesController:contentHash');
