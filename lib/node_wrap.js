  ...
  start: function () {
    ...
    var cb = function (err, result) {
      if (err) {
        ...
        BT.Logger.error("error found when executing BT.AsyncWrapper");
        BT.Logger.error("method: " + me.method);
        BT.Logger.error("error: " + util.inspect(err));
      }
      if (me.method === "exec") {
        BT.Logger.info("stdout: " + result);
      }
      SC.RunLoop.begin();
...
BT.AsyncManager = SC.Object.create({
    ...
    exec('ulimit -n', function (err, stdout, stderr) {
      ...
      if (err === null) {
        ...
        if (m > 0) {
          ...
          BT.Logger.debug("number of max files: " + me.maxOpen);
        }
      }
      else {
        BT.Logger.error('BT.AsyncManager: Error calculating maximum of simultaneous open files');
      }
    });
  },
  ...
  schedule: function (req) {
    BT.Logger.debug("BT.AsyncManager#schedule: in flight %@, removing req: %@".fmt(this._inFlight.length, req.args));
    this._queue.push(req);
    this.parseQueue();
  },

  ready: function (req) {
    BT.Logger.debug("BT.AsyncManager#ready: in flight %@, removing req: %@".fmt(this._inFlight.length, req.args));
