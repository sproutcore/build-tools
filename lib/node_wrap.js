...
BT.AsyncWrapper = SC.Object.extend({
  ...
  start: function () {
    ...
    var cb = function (err, result) {
      if (err) {
        ...
        SC.Logger.log("error found when executing BT.AsyncWrapper");
        SC.Logger.log("method: " + me.method);
        SC.Logger.log("error: " + util.inspect(err));
      }
      if(me.method === "exec"){
        SC.Logger.log("stdout: " + result);
      }
      SC.RunLoop.begin();
  ...
  result: function () {
    ...
    var ret = resultArgs[0] ? [{ error: true, err: err }].concat(retArgs) : retArgs;
  ...
  _guessMaxOpen: function () {
    ...
    exec('ulimit -n', function (err, stdout, stderr) {
      ...
      if (err === null) {
        ...
        SC.Logger.log('BT.AsyncManager: Error calculating maximum of simultaneous open files');
      }
    });
  },
