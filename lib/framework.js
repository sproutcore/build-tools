...
BT.Framework = SC.Object.extend({
  ...
  // the idea is that every fw instance does its own gulp
  // that can lead to issues if we are not careful with task naming
  // as far as I understand it, those tasks are global
  // the problem specifically would be that a task would contain
  // configuration specific to the framework...
  ...
  setupGulpPatterns: function(){
    ...
    if(this.isFrameworkBundle || this.isWrapperFramework) return;
    ...
    scriptsPattern = this.get('scriptExtensions').map(function(ext){
