...
BT.Framework = SC.Object.extend({
  ...
  minifyScripts: function () {
    if (BT.runMode === "debug") return false;
    else return true;
  }.property(),
  ...
  includeTests: function () {
    if (BT.runMode === "debug") return true;
    else return false;
  }.property(),
  ...
  initFramework: function () {
    //SC.Logger.log('initFramework for ' + this.get('ref'));
  ...
  targets: function () {
    ...
    var url = "/" + this.getPath('belongsTo.name') + "/" + this.get('fullname');
    return {
      kind: 'framework',
      name: "/" + this.get('fullname'),
      //link_docs: "",
      link_root: url,
      link_tests: url + "/tests/-index.json"
    };
  }.property().cacheable(),
