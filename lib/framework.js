...
BT.Framework = SC.Object.extend({
  ...
  init: function(){
    ...
    this.scripts = BT.FrameworkScriptsController.create({
      //contentBinding: this.files.bind('scripts')
    });
  ...
  scanFiles: function(){
    ...
    //var basicPatterns = [pathlib.join("**","*")];
    //if(!this.includeFixtures) basicPatterns.push("!" + pathlib.join("fixtures","**","*"));
    //if(!this.includeTests) basicPatterns.push("!" + pathlib.join("tests","**","*"));
    var p = pathlib.join(this.get('path'), "**", "*");
    var g = require('glob').Glob(p,{ sync: true, cwd: this.get('path') });
