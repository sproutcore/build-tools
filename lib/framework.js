  ...
  */
  buildNumber: function () {
    var contentHash = this.get('contentHash'),
      ...
      buildNumberPath = BT.serverConfig.get('buildNumberPath');

    if (!buildNumberPath) {
      SC.Logger.error("Cannot compute the buildNumber as buildNumberPath is not defined for the app '%@'.".fmt(app.get('name')));
      return contentHash;
    }
    ...
    if (!name) {
      SC.Logger.error("Could not find either a ref or a name the framework '%@'.".fmt(this.get('path')));
      return contentHash;
    }
