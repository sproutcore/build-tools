...
BT.Framework = SC.Object.extend({
  ...
  path: function () {
    ...
    //BT.Logger.debug("ref is " + this.get('ref'));
    //BT.Logger.debug("ret is " + ret);
    return ret;
  ...
  setupFSEvents: function () {
    //BT.Logger.debug("setupFSEvents for path: " + this.get('path'));
    ...
    //   BT.Logger.debug("fsevent: " + SC.inspect(arguments));
    // });
    watcher.on('change', function (path, info) {
      //BT.Logger.debug("fswatcher onChange: " + require('util').inspect(arguments));
      SC.RunLoop.begin();
      switch (info.event) {
        case "created":
  ...
  fsDirDidChange: function (directory, event, filename) {
    BT.Logger.debug("BT.Framework#fsDirDidChange");
    var fslib = require('fs');
    var pathlib = require('path');
    if (!filename) {
