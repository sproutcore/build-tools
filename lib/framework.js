...
BT.Framework = SC.Object.extend({
  ...
  //   else return false;
  // }.property(),
  includeTests: false, // gets overridden by the app config
   ...
   */
  dependencies: null,
  belongsTo: null, // to which app this framework belongs
  ...
  path: function () {
    ...
    //SC.Logger.debug("ref is " + this.get('ref'));
    //SC.Logger.debug("ret is " + ret);
    return ret;
  ...
  initFramework: function () {
    //SC.Logger.debug('initFramework for ' + this.get('ref'));
  ...
  scanFiles: function (opts) {
    ...
    if (app && !app.get('languages')) app.languagesToBuild = languages;
  ...
  setupFSEvents: function () {
    //SC.Logger.debug("setupFSEvents for path: " + this.get('path'));
    ...
    //   SC.Logger.debug("fsevent: " + SC.inspect(arguments));
    // });
    watcher.on('change', function (path, info) {
      //SC.Logger.debug("fswatcher onChange: " + require('util').inspect(arguments));
      SC.RunLoop.begin();
      switch (info.event) {
        case "created":
  ...
  fsDirDidChange: function (directory, event, filename) {
    SC.Logger.debug("BT.Framework#fsDirDidChange");
    var fslib = require('fs');
    var pathlib = require('path');
    if (!filename) {
      ...
      SC.Logger.error("You are using an OS which doesn't provide file names with filenames with events,");
      SC.Logger.error("You will have to restart the build tools for the changes to take effect.");
      SC.Logger.error("Please report which OS this is exactly...");
  ...
  fsDidCreateFile: function (filename) {
    SC.Logger.debug("fsDidCreateFile: " + filename);
  ...
  fsDidChangeFile: function (filename) {
    SC.Logger.debug("fsDidChangeFile: " + filename);
    var f = this.get('files').findProperty('path', filename);
    if (f) f.fileDidChange();
  },

  fsDidRenameFile: function (file, newFilename, calltype) {
    //SC.Logger.debug("arguments to this function: " + require('util').inspect(arguments));
    SC.Logger.debug("fsDidRenameFile: " + newFilename);
  ...
  findResourceFor: function (filename, basepath) {
    //SC.Logger.debug('findResources for filename: ' + filename);
