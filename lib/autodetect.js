...
var AppConfigParser = SC.Object.extend({
  ...
  takeNext: function(){
    ...
    p = path.join(p,'sc_config.json');
    async.exec(fs.readFile,p, { encoding: "utf8"}).notify(this,'nextDidRead', p, fw);
  },

  nextDidRead: function(result, args){
    ...
    else { // no config file, just insert a normal one
      data = { path: args[0] };
    }
    if(SC.typeOf(args[1]) === SC.T_HASH){
  ...
  rootState: SC.State.design({
    ...
    PARSEAPPS: SC.State.design({
      ...
      parseApp: function(appName){
        ...
        currApp.forEach(function(conf, confIndex){
          var p;
          if(!conf.frameworks) return;
          var curFws = namedHashToArray(conf.frameworks); // will convert if necessary
          if(curFws.length === 0) return;
          else {
            this._appsData[appName].fws[confIndex] = curFws.length; // amount of fws to handle
    ...
    CHECKSPROUTCORE: SC.State.design({
      ...
      detectSproutcoreDidRespond: function(result){
        if(SC.ok(result)){
          ...
          util.log('no sproutcore detected in frameworks folder');
        }
        cb(null,allConfigs);
      }
    })
