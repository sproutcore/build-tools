  ...
  rootState: SC.State.design({
    ...
    PARSEAPPS: SC.State.design({
      ...
      ready: function(target,key,value,rev){
        ...
        if(target.get(key)){
          target.removeObserver(this,'ready'); // remove the observer
        }
        ...
        Object.keys(this.appConfig).forEach(function(name){
          if(this.appConfig[name].length !== this._appsData[name].configs.length) ready = false;
          if(!this._appsData[name].configs.everyProperty('done')) ready = false;
        },this);
