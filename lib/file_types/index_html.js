...
BT.IndexHtmlFile = BT.File.extend({
  ...
  */
  rawContent: function () {
    ...
    var template;
    var deps = this.get('resourceDependencies');
    var belongsTo = this.get('belongsTo');

    if (belongsTo.get('indexHtmlTemplate')) { // try to load it
      var indexHtmlPath = pathlib.join(BT.projectPath, belongsTo.get('indexHtmlTemplate'));

      try {
        template = fslib.readFileSync(indexHtmlPath);
      }
      catch (e) {
        if (e.code === "ENOENT") {
          BT.Logger.warn("The buildtools could not find the configured template '%@', falling back to the default one".fmt(indexHtmlPath));
        }
        else throw e;
      }
    }
    ...
    try {
      ...
      ret = ejs.render(template.toString(), {
        ...
        sc_static: function (fn) {
          ...
          return file.get(app.get('doRelativeBuild') ? 'relativeUrl' : 'url');
        }
      });
