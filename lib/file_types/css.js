...
BT.CSSFile = BT.File.extend({
  ...
  minify: function (css) {
    ...
    try {
      ret = require('lightningcss').transform({
        filename: 'style.css',
      ...
      if (err.loc && err.loc.line) {
        lines = css.split('\n');
        start = Math.max(0, err.loc.line - 6);
        end = Math.min(lines.length, err.loc.line + 5);
        SC.Logger.error('Failed to compile CSS at line ' + err.loc.line + ', column ' + err.loc.column + '):');
        for (i = start; i < end; i++) {
          marker = (i === err.loc.line - 1) ? ' >>> ' : '     ';
          SC.Logger.error(marker + (i + 1) + ': ' + lines[i]);
        }
      }
      delete err.source;
      throw err;
  ...
  _parseSass: function (css) {
    ...
    var includePaths = [curPath];
    var ret;

    if (BT.runBenchmarks) SC.Benchmark.start('cssFile:sass');

    try {
      var result = sass.compileString(css, {
      ...
      BT.Logger.error("Sass compilation error: " + e.message);
    }
