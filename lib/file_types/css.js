...
BT.CSSFile = BT.File.extend({
  ...
  // h
  has2x: false, // flag to show whether this file also can have an 2x content
  ...
  url2x: function () {
    ...
    urlTemplate = (urlTemplate[urlTemplate - 1] !== "/") ? urlTemplate + "/" : urlTemplate;
    var opts = {
    ...
    lines.forEach(function (line) {
      ...
      if (match) {
        ...
        if (fn.indexOf(":") > -1) {
          ...
          var fwref = fn.substr(0, fn.lastIndexOf(":"));
        ...
        //var replaceURL = pathlib.join(app.get('name'), f.get('relativePath'));
        var replaceURL = f.get('url');
        ...
        //SC.Logger.log("BT.CssFile#handleStatic: replacing %@ with %@".fmt(match[0], replaceURL));
        ret.push(line.replace(scstaticRegex, 'url("%@")'.fmt(replaceURL)));
      }
      else {
        ret.push(line);
      }
    }, this);
    // no need for KVO, will be read only once by the savings process.
...
//  -moz-transition-duration: 0.5s;
  ...
  create_slice: function (opts) {
    ...
    if (file) { // this means an x2 file has been found, tag the current file that we are x2
      this.set('has2x', true);
    }
    ...
    if (file.get('extension') === "gif") {
      SC.Logger.log('You are trying to apply slicing on a gif. This is not supported');
      return;
    }
    //SC.Logger.log("for slice in css_file: " + this.get('path') + " using file: " + file.get('path'));
    opts.file = file;
  ...
  replacer: function (match, opts) {
    ...
    if (m.indexOf("slices") >= 0) { // slice or slices,
      ...
      var dataurl = this.slice_image(slice, slice.file.get('content'));
  ...
  content: function () {
    // this.set('has2x', false);
    // var c = this.parseContent({ x2: true });
    // if (this.get('has2x')) {
    //   // if has2x then files with 2x support have been found and possibly turned into dataurls
    //   // this content cannot be set as normal, so instead set it to content2x, and return the
    //   // content without the 2x option instead.
    //   this.set('content2x', c);
  ...
  content2x: null, // content when also 2x
