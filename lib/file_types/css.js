...
BT.CSSFile = BT.File.extend(BT.SliceMixin, {
  ...
  parseContent: function (opts) {
    ...
    var themeDef = this.findIncludeFile("_theme.css");
    ...
    if (partialIncludes) {
      ...
      var partialContents = partialIncludes.map(function (p) {
    ...
    if (!themeDef) { // if a theme definition cannot be found, add it just in case
  ...
  findIncludeFile: function (filename) {
    ...
    var frameworkFiles = this.getPath('framework.stylesheets');
    var workdir = pDir;
  ...
  findSassPartials: function () {
    ...
    var p = this.get('path');
    var frameworkDir = this.getPath('framework.path');
    var frameworkFiles = this.getPath('framework.files.sassPartials');
    var done = false;
    var pathlib = require('path');
    ...
    var findPartial = function (f) {
      ...
      if (tmpfn.indexOf(workdir) > -1) {
        if (f.get('basename')[0] === "_" && f.get('extname') === ".css") {
  ...
  loadImage: function (path, filebuffer) {
    ...
    if (!img.complete) {
      BT.Logger.warn('no filebuffer found for ' + path);
    }
    return img;
  },
