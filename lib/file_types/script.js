    ...
    lines.forEach(function (line) {
      ...
      if (match) {
        ...
        if (fn.indexOf(":") > -1) {
          ...
          if (!crossFw) {
            SC.Logger.log("BT.ScriptFile#handleStatic: %@ is referred in %@ but this framework is still not loaded.".fmt(match[0], this.get('path')));
            ret.push(line);
            return;
          }
          fwForFile = crossFw;
        ...
        if (opts.length === 0) {
          SC.Logger.log("BT.ScriptFile#handleStatic: found no files for %@ in file %@".fmt(match[0], this.get('path')));
        ...
        f = opts[0];
        if (opts.length > 1) {
          SC.Logger.log("BT.ScriptFile#handleStatic: found multiple files for %@ in file %@, taking the first (%@)".fmt(match[0], this.get('path'), f.get('path')));
        }
        // now we have a match, we need an url
        //ret.push(line.replace(scstaticRegex, '"%@"'.fmt(pathlib.join(app.get('name'), f.get('relativePath')))));
        ret.push(line.replace(scstaticRegex, '"%@"'.fmt(f.get('url'))));
