...
BT.ScriptFile = BT.File.extend({
  ...
  minify: function (str) {
    ...
    const minifySync = syncRpc(pathlib.join(require.main.path, 'terser-worker.js'));
    const terser_config = {
      warnings: true,
      output: {
        max_line_len: 1000
    ...
    try {
      result = minifySync([str, terser_config]);
    }
    catch (error) {
      BT.Logger.error("Error while minifying the script file '%@': ".fmt(this.get('path')));
      BT.Logger.error(error);
    }

    if (result.warning) {
      BT.Logger.warn("Warning while minifying the script file '%@': ".fmt(this.get('path')));
      BT.Logger.warn(result.warning);
    }
    if (result.error) {
      BT.Logger.error("Error while minifying the script file '%@': ".fmt(this.get('path')));
      BT.Logger.error(result.error);
      return str+'console.error("minification has failed");';
    }
  ...
  */
  handleRunModeComments: function (src) {
    ...
    (app.envNames || []).forEach(function(envName) {
      if (BT.envName !== envName) {
        regex.push('env\\.'+envName);
      }
    });

    var startregex = new RegExp('@if\\s?\\('+regex.join('|')+'\\)');
    //var endregex = /@endif/;
    var lines = src.split("\n"), ret = [];
    var insideIfDebug;
